...
/**
 * The `/edit` route: a full-page form for the most commonly edited quotation
 * fields — document meta, cover, the two parties (Prepared By / Billed To), and
 * the investment breakdown (line items → total → advance).
 *
 * Every input reads from `inv` and writes through `set`; the printed document
 * reads the same `inv`, so edits flow live in both directions.
 */
export function EditView() {
  const { inv, set } = useInvoice();
  const { saveState } = useUI();
  const total = inv.invest.items.reduce((sum, item) => sum + item.amount, 0);

  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-y-auto">
      {/* Page header */}
      <div className="bg-card/40 border-b">
        <div className="mx-auto flex w-full max-w flex-wrap items-center justify-between gap-3 px-4 py-4 sm:px-6">
          <div className="min-w-0">
            <h1 className="font-heading text-lg font-semibold tracking-tight">Edit Details</h1>
            <p className="text-muted-foreground text-sm">
              Changes save automatically and update the document live.
            </p>
          </div>
          <div className="flex items-center gap-3">
            <SaveState state={saveState} />
            <Link href="/" className={cn(buttonVariants({ variant: "default", size: "sm" }))}>
              <ArrowLeftIcon /> Back to document
            </Link>
          </div>
        </div>
      </div>
      ...
      {/* Form body */}
      <div className="mx-auto w-full max-w gap-5 p-4 sm:p-6 lg:columns-2">
        <Section title="Document" icon={FileTextIcon}>
          <div className="grid grid-cols-2 gap-3">
            <Field label={inv.meta.docNoLabel || "Document No."}>
              <Input value={inv.meta.docNo} onChange={(e) => set((d) => (d.meta.docNo = e.target.value))} />
            </Field>
        ...
        <Section title="Cover" icon={LayoutTemplateIcon}>
          <Field label="Eyebrow">
        ...
        <Section title="Prepared By" icon={UserRoundIcon}>
        ...
        <Section title="Billed To" icon={Building2Icon}>
          <div className="grid grid-cols-[1fr_1.4fr] gap-3">
            <Field label="Label">
        ...
        <Section title="Investment Breakdown" icon={ReceiptTextIcon}>
          <div className="grid grid-cols-2 gap-3">
          ...
          <div className="grid gap-3">
            {inv.invest.items.map((item, i) => (
              <div key={i} className="bg-background grid gap-2 rounded-lg border p-3">
                ...
                <Field label="Name">
                  <Input value={item.name} onChange={(e) => set((d) => (d.invest.items[i].name = e.target.value))} />
                </Field>
                <Field label="Description">
                  <Input value={item.desc} onChange={(e) => set((d) => (d.invest.items[i].desc = e.target.value))} />
                </Field>
                <div className="grid grid-cols-3 gap-2">
                  <Field label="Qty">
                    <Input value={item.qty} onChange={(e) => set((d) => (d.invest.items[i].qty = e.target.value))} />
                  </Field>
                  <Field label="Unit">
                    <Input value={item.unit} onChange={(e) => set((d) => (d.invest.items[i].unit = e.target.value))} />
                  </Field>
                  <Field label="Amount">
                    <Input
                      type="number"
...
function Section({
  ...
  return (
    <section className="bg-card mb-5 grid break-inside-avoid gap-3 rounded-xl border p-4 shadow-xs sm:p-5">
      <h2 className="flex items-center gap-2 text-sm font-semibold">
        <span className="bg-accent text-accent-foreground flex size-6 shrink-0 items-center justify-center rounded-md">
